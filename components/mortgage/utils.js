  ...
  for(let i = 1; i <= mortgageTerm; i++) {
    ...
    //loop each month of the year as interest is calculated monthly
      ...
      yearDataObject.push({
          ...
          interestPaid: parseFloat(interestPaidMonthlyToYearlyIncrementer.toFixed(0)),
          interestPaidToDate: parseFloat(interestPaidToDate.toFixed(0)),
          principalRepaid: parseFloat(monthlyPrincipalRepaidToYearlyIncrementer.toFixed(0)),
          principalRepaidToDate: parseFloat(principalRepaidToDate.toFixed(0))
      });	
    }
